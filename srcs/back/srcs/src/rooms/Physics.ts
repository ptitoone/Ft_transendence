import { Ball, Paddle, PONG_HEIGHT, PONG_WIDTH } from './Schema'
import { toRadians, map } from './utils'
...
export class Physics {
	...
	constructor(ball: Ball, leftPaddle: Paddle, rightPaddle: Paddle, options?: PhysicsOptions) {
		const { ballSpeed = 0.5, ballAngle = 0, paddleSpeed = 0.3 } = options || {};
	...
	private topBottomCollision() {
		if (this.ball.y < 0 + Ball.radius || this.ball.y > PONG_HEIGHT - Ball.radius) {
			this.ySpeed = -this.ySpeed;
		}
	}

	public checkRightWall() {
		if (this.ball.x - Ball.radius > PONG_WIDTH) {
			return true;
		}
		return false;
	}
	...
	private movePaddle(paddle: Paddle, direction: PaddleDirection, deltaTime: number) {
		switch (direction) {
			...
			case PaddleDirection.DOWN:
				const maxY = PONG_HEIGHT - Paddle.height / 2;
