...
let generateCode = (secret: string, counter:number): number => {
...
let validateCode = (code: number, secret: string): boolean => generateCode(secret, Math.floor(Date.now() / 30000)) === code;
let generateGoogleUri = (user: string, secret: string): string => `otpauth://totp/Ft_transcendence:${user}?secret=${secret}`;
